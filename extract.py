...
def cropImage(image, target):
	...
	if (height <= 2 or width <= 2):
		return None
	#resize
	while (height < targetSize and width < targetSize):
		height *= 1.25
		width *= 1.25
	while (height > targetSize or width > targetSize):
		height /= 1.25
		width /= 1.25
	height = int(height / 2) * 2
	width = int(width / 2) * 2
	resized_image = cv2.resize(croppedImage, (width, height))
	result = np.zeros((targetSize, targetSize, 3), dtype=np.uint8)
	mid = int(targetSize / 2)
	h2 = int(height / 2)
	w2 = int(width / 2)
	#pad
	result[mid - h2 : mid + h2, mid - w2 : mid + w2] = resized_image
	return result
