...
def cnn_model_fn(features, labels, mode):
	...
	net = tf.layers.conv2d(input_layer, 32, [5, 5], strides=[2, 2], padding='SAME')
	...
	original = tf.reshape(features, [-1, 28, 28, 1])
	input_layer = tf.reshape(features, [-1, 28 * 28 * 1])

	dense1 = tf.layers.dense(inputs=input_layer, units=32, activation=tf.nn.tanh)
	dense2 = tf.layers.dense(inputs=dense1, units=32, activation=tf.nn.tanh)
	dense3 = tf.layers.dense(inputs=dense1, units=28 * 28 * 1, activation=tf.nn.tanh)
	result = tf.reshape(dense3, [-1, 28, 28, 1])
	...
	loss = tf.losses.mean_squared_error(input_layer, dense3)
...
def main(unused_argv):
	...
	gender_classifier = tf.estimator.Estimator(
		model_fn=cnn_model_fn, model_dir="model/encoder")
